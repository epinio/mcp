...
func adoptAppHandler(_ *client.Client) func(
	context.Context,
	*mcp.CallToolRequest,
	AdoptAppInput,
) (*mcp.CallToolResult, AdoptAppOutput, error) {
	return func(
		ctx context.Context,
		...
		if input.Namespace == "" || input.DeploymentName == "" {
			return nil, AdoptAppOutput{}, fmt.Errorf(
				"namespace and deployment_name are required",
			)
		}
		...
		err = k.LabelDeploymentPodTemplate(
			ctx,
			input.Namespace,
			input.DeploymentName,
			podLabels,
		...
		if serviceName != "" {
			...
			if err != nil {
				log.Printf(
					"adopt_app: failed to label service %q/%q (non-fatal): %v",
					input.Namespace,
					serviceName,
					err,
				)
...
func reconcileAppHandler(_ *client.Client) func(
	context.Context,
	*mcp.CallToolRequest,
	ReconcileAppInput,
) (*mcp.CallToolResult, ReconcileAppOutput, error) {
	return func(
		ctx context.Context,
		req *mcp.CallToolRequest,
		...
		if input.Namespace == "" || input.Name == "" {
			return nil, ReconcileAppOutput{}, fmt.Errorf(
		...
		observedImage := primaryContainerImage(
		...
		currentImage, _, _ := unstructured.NestedString(
			app.Object,
			"spec",
			"imageurl",
		)
		currentContainer, _, _ := unstructured.NestedString(
			app.Object,
			"spec",
			"origin",
			"container",
		)
		currentRoutes, _, _ := unstructured.NestedStringSlice(
			app.Object,
			"spec",
			"routes",
		)
		currentAnnotated := app.
			GetAnnotations()[EpinioAdoptedAnnotation] == "true"
		...
		if !currentAnnotated {
			changes = append(changes, fmt.Sprintf(
				"annotations[%s]: set to \"true\"",
				EpinioAdoptedAnnotation,
			))
		}
...
func releaseAppHandler(_ *client.Client) func(
	context.Context,
	*mcp.CallToolRequest,
	ReleaseAppInput,
) (*mcp.CallToolResult, ReleaseAppOutput, error) {
	return func(
		ctx context.Context,
		req *mcp.CallToolRequest,
		input ReleaseAppInput,
	) (*mcp.CallToolResult, ReleaseAppOutput, error) {
		if input.Namespace == "" || input.Name == "" {
			return nil, ReleaseAppOutput{}, fmt.Errorf(
				"namespace and name are required",
			)
		}
		...
		err = k.RemoveDeploymentMetadata(
			ctx,
			input.Namespace,
			input.Name,
			labelKeys,
		...
		if err != nil && !apierrors.IsNotFound(err) {
			log.Printf(
				"release_app: strip deployment metadata (non-fatal): %v",
				err,
			)
		}

		if dep, err := k.GetDeployment(
			ctx,
			input.Namespace,
			input.Name,
		); err == nil {
			svc := discoverServiceForDeployment(
				ctx,
				k,
				input.Namespace,
				dep.Spec.Selector.MatchLabels,
			...
			if svc != "" {
				...
				if err != nil && !apierrors.IsNotFound(err) {
					log.Printf(
						"release_app: strip service metadata (non-fatal): %v",
						err,
					)
				}
			}
...
func discoverServiceForDeployment(
	ctx context.Context,
	k *KubeClient,
	namespace string,
	depSelector map[string]string,
) string {
